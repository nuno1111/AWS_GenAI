...
import src.rag_chatbot_lib as glib #reference to local lib script

def rag_chatbot_app_main():
    st.title('챗봇 RAG 데모')
    ...
    if 'vector_index' not in st.session_state: #벡터 인덱스가 아직 생성되지 않았는지 확인합니다. 
        with st.spinner("주문아이템테이블 PDF 인덱싱중..."): #이 블록의 코드가 실행되는 동안 스피너를 표시합니다.
            st.session_state.vector_index = glib.get_index() #지원 라이브러리를 통해 인덱스를 검색하고 앱의 세션 캐시에 저장합니다.
    st.markdown("Prompt : 주문아이템 테이블에 대해서 상세히 설명해주세요.") 
