...
import src.video_attribute_lib_v2 as glib

# st.set_page_config(layout="wide", page_title="홈쇼핑 속성 추출")

frame_prompt = """- 해당 이미지는 홈쇼핑 방송에서 한 frame을 캡처한 것입니다.
- 다음은 해당 이미지에서 추출한 ocr_text 정보입니다.
<ocr_text/>
- 이미지와 ocr_text에서 "가격" 정보가 보이지 않는다면 <result>없음</result>을 결과값으로 리턴해주세요.

- 가격정보가 있다면 이미지와 ocr을 참조하여 "대표상품명/가격/행사카드할인율"을 추출해주세요.
...
<선택명유의사항>
- '선택[숫자] [선택명]' 이렇게 표현되어 있습니다.  ex) 선택1 xx 박스
...
- 추출해야 될 속성명은 다음과 같습니다.
  1. 대표상품명 : 해당상품명을 추출해주세요. 이미지 가장 상단과, OCR 가장 앞에 대표상품명이 위치합니다.
...
- 여러 frame에 대부분 "없음"으로 나오는 항목은 "없음"은 무시하고, 값이 있는 데이터에서 추출해주세요.
</instruction>
...
def video_attribute_app_main():
    ...
    with col1:
        ...
        # st.subheader("분석정보")
        target_min = st.number_input("분석 분량을 선택해주세요(분)", min_value=1, max_value=60, step=1, value=5)
